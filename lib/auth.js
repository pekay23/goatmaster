...
if (!JWT_SECRET) {
  throw new Error(
    ...
    'Set it to a strong random value (recommend: openssl rand -base64 64).'
  );
}
if (JWT_SECRET.length < 32) {
  throw new Error(
    '[FATAL] JWT_SECRET is too short (must be ≥ 32 characters). ' +
...
// Admin-only helper — returns 403 if user is not an admin
