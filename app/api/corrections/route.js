...
export async function GET(request) {
  try {
    ...
    const { rows } = await query(
      'SELECT * FROM corrections WHERE owner_id = $1 AND updated_at > $2 ORDER BY updated_at ASC LIMIT $3',
...
export async function POST(request) {
  try {
    ...
    const { id, correction_type, source_goat_id, target_goat_id, notes, status } = data;
    ...
    const { rows } = await query(
      `INSERT INTO corrections (id, correction_type, source_goat_id, target_goat_id, notes, status, owner_id) 
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       ON CONFLICT (id) DO UPDATE SET
       notes = EXCLUDED.notes, status = EXCLUDED.status, updated_at = CURRENT_TIMESTAMP
       RETURNING *`,
      [id || null, correction_type, source_goat_id || null, target_goat_id || null, notes || null, status || 'pending', owner_id]
    );
